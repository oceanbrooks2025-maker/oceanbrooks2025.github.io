...
系统选了 **Ubuntu Server LTS**，为了省资源没装桌面环境，全靠 SSH 远程管理。
...
* **Cockpit：** 装了个 Web 管理面板（把端口改到了 9099 避开 Kodi），主要是为了看 CPU 温度和内存占用，以及便捷查看和控制进程状态。
* **Syncthing：** 搭建了私有同步云。
* **策略：** 手机端“只发送”，服务器端“只接收”且**忽略删除**。这样手机为了腾空间删掉照片，服务器上还会保留备份，实现了备份功能。
...
1. **Docker 化：** 准备把能容器化的服务都搬进 Docker，保持系统洁净。
